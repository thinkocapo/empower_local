...
function Products({ frontendSlowdown, backend, productsApi, productsExtremelySlow, productsBeError, addToCartJsError }) {
  ...
  // the "Products" link while already here -- so we re-fetch products each time.
  ...
  */
  ...
  useEffect(() => {
     // Reset per-load measurement state so each navigation to /products is a
     // fresh fetch + a fresh products.load_duration timing.
     startTime.current = performance.now();
     productsRendered.current = false;
     try {
